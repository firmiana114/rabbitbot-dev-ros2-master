...
- 2026-07-07 台词配置补充：移除左侧“领导称呼”子页面入口，将领导称呼加载/保存面板移动到“点位台词”子页面内，作为点位台词配置的子板块。
- 2026-07-07 点位坐标补充：点位台词表格操作列新增“更新为机器人当前位置”按钮，使用 `/api/status` 最近一次定位位姿回填该行坐标 JSON；opening 行禁用该按钮，普通行回填后仍需点击保存点位台词才会写入配置。
- 2026-07-07 操作列布局补充：点位台词表格操作列改为横向按钮组，“更新为机器人当前位置”和删除按钮默认同一行显示，窄屏时由表格横向滚动或移动端自然换行承接。
- 2026-07-07 称呼文案补充：点位台词页面里的“领导称呼”展示文案改为“嘉宾称呼”；底层配置键仍为 `variables.leader_calling`，不会自动改写 `conf/dialogue_0.json` 当前称呼值。
...
- 2026-07-07 领导称呼并入点位台词页面后运行同一组控制台测试，结果为 66 passed；相关 py_compile 和 JSON 校验通过。
- 2026-07-07 点位坐标回填按钮修改后运行同一组控制台测试，结果为 66 passed；相关 py_compile 和 JSON 校验通过。
- 2026-07-07 操作列按钮并排修改后运行同一组控制台测试，结果为 66 passed；相关 py_compile 和 JSON 校验通过。
- 2026-07-07 称呼展示文案修改后运行同一组控制台测试，结果为 66 passed；相关 py_compile 和 JSON 校验通过。

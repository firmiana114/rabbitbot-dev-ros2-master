...
- 本轮现场释放了导航桥接常用端口 `28180`：确认占用方为 `python3` 进程 PID `7283`，执行结束后复查 `ss -ltnp "sport = :28180"` 已无监听进程。
...
- `28180` 端口已在本轮释放；如再次启动 `start_nav_arm_bridge.sh` 仍提示占用，应重新用 `ss -ltnp "sport = :28180"` 定位新的监听进程。

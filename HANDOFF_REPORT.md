...
- 本轮本体语音测试：宿主机因 GLIBC 版本不匹配无法直接运行桥接程序，容器内 `unitree_g1_tts_bridge` 通过 `eno1` 探测音量成功返回 `ret=0, volume=100`；中文短句请求和音量设置均返回 `ret=0` 但现场未听到中文，官方 G1 音频示例返回 `play_state` 且英文段已被现场确认有声；随后再次用 `speaker=0`、音量 100 发送“中文播报测试，请确认是否听到。”也成功返回，说明本体喇叭和 Unitree 音频服务可用，但当前中文 TTS 通道存在“服务接收成功但不出声”的问题。
- 生成时间：2026-06-22
